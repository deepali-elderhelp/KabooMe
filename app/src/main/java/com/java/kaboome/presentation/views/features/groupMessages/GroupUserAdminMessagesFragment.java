...
public class GroupUserAdminMessagesFragment extends BaseFragment implements EasyPermissions.PermissionCallbacks, MessageInput.InputListener, MessageInput.TypingListener, MessageInput.AttachmentsListener,
    ...
    DownloadClickListener, UploadClickListener {
        ...
        private AdminMessagesViewModel messagesViewModel;
    ...
    public GroupUserAdminMessagesFragment() {
        ...
        public void onCreate(@Nullable Bundle savedInstanceState) {
            ...
            messagesViewModel = ViewModelProviders.of(this, new CustomViewModelProvider(group)).get(AdminMessagesViewModel.class);
            alreadyThere = false;

            String topicName = MessageGroupsHelper.getTopicName(group.getGroupId(), AppConfigHelper.getUserId(), MessageGroupsConstants.USER_ADMIN_MESSAGES);
...
//            });
            ...
            rootView = inflater.inflate(R.layout.fragment_group_admin_messages, container, false);
...
//        TextView input = rootView.findViewById(R.id.fr_gr_me_layout_chatbox);
            ...
            navController = NavHostFragment.findNavController(GroupUserAdminMessagesFragment.this);
...
//        firstLoad = false;
        ...
        public void onResume() {
            ...
            mainToolbar.setTitle("Admins of "+group.getGroupName());
...
//                        Long lastMessageTSForGroup = group.getAdminsLastAccessed();
...
//        startActivityForResult(openIntent, OPEN_REQUEST_CODE);
            ...
            args.putString("goingBackTo", "UserAdminMessages");
            navController.navigate(R.id.action_groupUserAdminMessagesFragment_to_messageAttachmentsDialog, args);
...
//                        Toast.makeText(MessagesActivity.this,"Yes, copy text",Toast.LENGTH_LONG).show();
                                GroupUserAdminMessagesFragment.this.adapter.copyToClipboard(getContext(), message.getMessageText());
                            }
                        });
...
//
        ...
        private void openAudio(Message message, Uri uri) {
            ...
            if(message.getAttachmentUploaded()) {
                ...
                navController.navigate(R.id.action_groupUserAdminMessagesFragment_to_audioViewerFragment, args);
            }
            else{
        ...
        private void openImage(Message message) {
            ...
            if(message.getAttachmentUri() != null && MediaHelper.doesUriFileExists(getContext().getContentResolver(), Uri.parse(message.getAttachmentUri()))){
                Bundle args = new Bundle();
                args.putSerializable("message", message);
                navController.navigate(R.id.action_groupUserAdminMessagesFragment_to_imageViewerFragment, args);
            }
            else{
        ...
        private void openVideo(Message message, Uri uri) {
            ...
            if(message.getAttachmentUploaded()){
                ...
                navController.navigate(R.id.action_groupUserAdminMessagesFragment_to_videoViewerFragment, args);
            }
            else{
...
//                public void onChanged(DomainUpdateResource domainUpdateResource) {
//                    if(DomainUpdateResource.Status.SUCCESS == domainUpdateResource.status){
//                        messagesViewModel.getDownloadedMessageAttachment().removeObservers(GroupUserAdminMessagesFragment.this);
//                        //viewmodel call to update the cache use case
//                        MessageTempDataHolder messageTempDataHolder  = (MessageTempDataHolder) domainUpdateResource.data;
...
//            if(EasyPermissions.hasPermissions(getContext(), perms)){
//                try {
//                    attachment = FileUtils.createAttachmentFileForMessage(message.getMessageId(), message.getGroupId(), AppConfigHelper.getUserId(), message.getAttachmentExtension(), message.getAttachmentMime());
//                } catch (IOException e) {
//                    e.printStackTrace();
//                    return;
//                }
...
//            }
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q){
                ...
                try {
                    attachment = FileUtils.createAttachmentFileForMessageAttachment(getContext(), message.getGroupId(), AppConfigHelper.getUserId(), message.getMessageId(),message.getAttachmentExtension(), message.getAttachmentMime());
//                messagesViewModel.startDownloadingAttachment(message.getMessageId(), message.getGroupId(), message.getSentAt(), message.getAttachmentExtension(), attachment.getAbsolutePath(), message.getAttachmentMime() );
                ...
                if(EasyPermissions.hasPermissions(getContext(), perms)){
                    ...
                    messagesViewModel.updateMessageForAttachment(message.getMessageId(), true, true, true, message.getAttachmentMime(), null);
                    try {
                        attachment = FileUtils.createAttachmentFileForMessage(message.getMessageId(), group.getGroupName(),AppConfigHelper.getUserId(), message.getAttachmentExtension(), message.getAttachmentMime());
                    } catch (IOException e) {
                        e.printStackTrace();
                        Log.d(TAG, "Exception - "+e.getMessage());
                        return;
...
//            File attachmentFile = FileUtils.copyAttachmentToApp(FileUtils.getUri(attachment), AppConfigHelper.getUserId(), message.getMessageId(), message.getGroupId(), getContext());
...
//            messagesViewModel.startUploadingAttachment(message.getMessageId(), message.getGroupId(), message.getSentAt(), message.getAttachmentExtension(), message.getAttachmentMime(), attachment);
        ...
        @Override
        public void onUploadClicked(Message message, java.io.File fileToUpload) {
            if(!NetworkHelper.isOnline()){
...
//        Toast.makeText(getContext(), "Show user Image bigger or something", Toast.LENGTH_SHORT).show();
            Bundle args = new Bundle();
            args.putSerializable("message", message);
            navController.navigate(R.id.action_groupUserAdminMessagesFragment_to_userImageDisplayFragment, args);
        }
    }
