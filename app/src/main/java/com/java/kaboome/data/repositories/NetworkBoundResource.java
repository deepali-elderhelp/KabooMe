...
public abstract class NetworkBoundResource<CacheObject, RequestObject> {
    ...
    public NetworkBoundResource(AppExecutors2 appExecutors) {
    ...
    private void fetchFromNetwork(final LiveData<CacheObject> dbSource){
        ...
        results.addSource(dbSource, new Observer<CacheObject>() {
            @Override
            public void onChanged(@Nullable CacheObject cacheObject) {
                Log.d(TAG, "going to call setting new value loading - line 76");
        ...
        results.addSource(apiResponse, new Observer<ApiResponse<RequestObject>>() {
            ...
            public void onChanged(@Nullable final ApiResponse<RequestObject> requestObjectApiResponse) {
                ...
                if(requestObjectApiResponse instanceof ApiResponse.ApiSuccessResponse){
                            ...
                            appExecutors.mainThread().execute(new Runnable() {
                                ...
                                public void run() {
                                    results.addSource(loadFromDb(), new Observer<CacheObject>() {
                                        @Override
                                        public void onChanged(@Nullable CacheObject cacheObject) {
                                            Log.d(TAG, "going to call setting new value - line 110");
                                            Log.d(TAG, "onChanged: cache object - "+cacheObject.toString());
                                            setValue(Resource.success(cacheObject));
                                        }
                                    });
                                }
                    ...
                    appExecutors.mainThread().execute(new Runnable() {
                        ...
                        public void run() {
                            results.addSource(loadFromDb(), new Observer<CacheObject>() {
                                @Override
                                public void onChanged(@Nullable CacheObject cacheObject) {
                                    Log.d(TAG, "going to call setting new value - line 126");
                                    setValue(Resource.success(cacheObject));
                                }
                            });
                        }
                    ...
                    //changed the following because the error is not being propogated to the upper layer
                    //some how the onChanged() for dbSource is not called at all.
...
//                    });
                    ...
                    results.addSource(loadFromDb(), new Observer<CacheObject>() {
                        @Override
                        public void onChanged(@Nullable CacheObject cacheObject) {
                            Log.d(TAG, "Error message "+((ApiResponse.ApiErrorResponse) requestObjectApiResponse).getErrorMessage());
                            setValue(
                                    Resource.error(
                                            ((ApiResponse.ApiErrorResponse) requestObjectApiResponse).getErrorMessage(),
                                            cacheObject
    ...
    private void setValue(Resource<CacheObject> newValue){
        if(results.getValue() != newValue){
            Log.d(TAG, "setValue: setting new value");
