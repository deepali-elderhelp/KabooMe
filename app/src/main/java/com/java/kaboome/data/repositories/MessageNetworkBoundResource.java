...
/**
 * This class only fetches the
 * @param <RequestObject>
 */


public abstract class MessageNetworkBoundResource<RequestObject> {

    private static final String TAG = "KMMessageNetworkBoundR";

    private AppExecutors2 appExecutors;
    private MediatorLiveData<Resource<RequestObject>> results = new MediatorLiveData<>();

    public MessageNetworkBoundResource(AppExecutors2 appExecutors) {
                ...
                if(requestObjectApiResponse instanceof ApiResponse.ApiSuccessResponse){
                    ...
                    appExecutors.diskIO().execute(new Runnable() {
                        ...
                        public void run() {
                            ...
                            postValue(Resource.success((RequestObject) processResponse((ApiResponse.ApiSuccessResponse)requestObjectApiResponse)));
...
//                    setValue(Resource.success((RequestObject) processResponse((ApiResponse.ApiSuccessResponse)requestObjectApiResponse)));
                    ...
                    Log.d(TAG, "onChanged: ApiErrorResponse, reason "+ ((ApiResponse.ApiErrorResponse) requestObjectApiResponse).getErrorMessage());
...
//                    setValue(new Resource<RequestObject>(Resource.Status.ERROR, null, ((ApiResponse.ApiErrorResponse) requestObjectApiResponse).getErrorMessage()));
    ...
    private void postValue(Resource<RequestObject> newValue){
        if(results.getValue() != newValue){
